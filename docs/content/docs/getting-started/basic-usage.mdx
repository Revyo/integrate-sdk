...
// Plugins automatically read GITHUB_CLIENT_ID, GMAIL_CLIENT_ID, etc. from environment
...
Plugins automatically read credentials from standard environment variables:
...
The `githubPlugin` automatically looks for `GITHUB_CLIENT_ID` and `GITHUB_CLIENT_SECRET` in your environment. Similarly, `gmailPlugin` looks for `GMAIL_CLIENT_ID` and `GMAIL_CLIENT_SECRET`.
