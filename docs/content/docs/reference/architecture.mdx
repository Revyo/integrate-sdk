...
```
1. Application calls typed method (e.g., client.github.createIssue())
2. Proxy converts method to tool name (e.g., github_create_issue)
3. Client validates tool is enabled
4. Client creates JSON-RPC request
5. Transport sends request to server
6. Server executes tool (OAuth happens here)
7. Server sends JSON-RPC response
8. Transport receives response
9. Client parses and returns result
```
