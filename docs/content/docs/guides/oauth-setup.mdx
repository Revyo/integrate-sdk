...
This page component handles the OAuth redirect from the provider and:
- Extracts the authorization code and state from the URL
- Sends them to the opener window (popup mode) or stores in sessionStorage (redirect mode)
- Redirects to your configured URL
- Displays a beautiful loading UI with spinner
- Handles errors gracefully

**That's it!** No need to manually handle OAuth callbacks or write error handling code.
...
A pre-built React component that handles OAuth redirects from providers.

```typescript
function OAuthCallbackPage(config?: OAuthCallbackHandlerConfig): JSX.Element
```

**OAuthCallbackHandlerConfig**:
```typescript
interface OAuthCallbackHandlerConfig {
...
**Alternative: Using createOAuthCallbackPage**:
```tsx
// app/oauth/callback/page.tsx
import { createOAuthCallbackPage } from 'integrate-sdk';
...
**What it does**:
- Handles OAuth redirects from providers (GitHub, Gmail, etc.)
- Extracts authorization code and state from URL parameters
- For popup mode: Sends data to opener window via `postMessage`
- For redirect mode: Stores data in sessionStorage and redirects
- Handles errors and redirects to error page
- Displays a beautiful gradient loading UI with animated spinner
