...
const { POST, GET } = handler.createCatchAllRoutes({
  redirectUrl: "/dashboard",
...
**Note**: This method requires a separate OAuth redirect handler file. Use `createCatchAllRoutes()` for a simpler setup.

### createOAuthRedirectHandler()

**Note**: Not needed when using `createCatchAllRoutes()` - only needed with `createRoutes()`.
