...
export const GET = createNextOAuthCallbackHandler({
  redirectUrl: '/dashboard', // Optional: Where to redirect after OAuth (default: '/')
});
```

This route handles the OAuth redirect from the provider and:
...
### createNextOAuthCallbackHandler()

Creates a pre-built OAuth callback route handler that processes OAuth redirects from providers.

```typescript
function createNextOAuthCallbackHandler(config?: OAuthCallbackHandlerConfig): {
...
- Displays a loading UI while processing
