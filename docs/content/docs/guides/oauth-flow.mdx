...
The Integrate SDK provides a pre-built callback handler that you can use:
...
export const GET = createNextOAuthCallbackHandler({
  redirectUrl: '/dashboard', // Where to redirect after OAuth completes
});
```
...
- Displaying errors if OAuth fails
