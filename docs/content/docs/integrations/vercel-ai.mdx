...
The Vercel AI SDK is commonly used server-side in API routes and serverless functions. Here's how to use it with the Integrate SDK on the server:

### Flow Overview

1. **Client**: User authenticates via OAuth, tokens stored in localStorage
2. **Client → Server**: Tokens sent in request headers
3. **Server**: Uses tokens to execute tools with user's permissions
4. **Response**: AI results returned to client

### Server-Side Example

```typescript
// app/api/ai/route.ts (Next.js API Route)
...
export async function POST(req: Request) {
  // 1. Get provider tokens from request header
  ...
  // 2. Create server client with OAuth secrets
...
```typescript
// Client-side code to send tokens to server
...
6. **Connection Cleanup**: Always disconnect after request completes
