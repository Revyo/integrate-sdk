...
Setting up Vercel AI integration requires **5 files**:
...
Create a providers component that injects tokens into AI SDK requests:
    ...
    <div className="flex flex-col h-screen">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message) => (
          ...
          >
            <div className="inline-block max-w-2xl">{message.content}</div>
          </div>
        ))}
      </div>
      <form
