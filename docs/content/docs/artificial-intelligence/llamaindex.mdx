...
Install the Integrate SDK and LlamaIndex packages:
...
Setting up LlamaIndex integration requires **3 files** (or **5 files** if not using a database):
...
Create an agent route that converts MCP tools to LlamaIndex format:

```typescript
// app/api/agent/route.ts
import { serverClient } from "@/lib/integrate";
import { getLlamaIndexTools } from "integrate-sdk/server";
...
Use LlamaIndex in any component:
