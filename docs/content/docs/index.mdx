...
- 🔌 **Plugin-Based Architecture** - Enable only the integrations you need, reducing bundle size and surface area
...
- 🛠️ **Extensible** - Add any integration supported by the Integrate MCP server with simple plugin configuration
- 📦 **Zero Dependencies** - Lightweight implementation with no external dependencies
- 🔄 **Automatic Re-authentication** - Handle token expiration gracefully with built-in re-auth flows
- 🎯 **Tool Discovery** - Automatically discover available tools based on your enabled plugins
...
You configure the SDK with plugins for each integration you want to use—GitHub for repository management, Gmail for email, Notion for knowledge base access, and so on. Each plugin knows what OAuth credentials it needs and what tools it provides.
...
The SDK queries the server for available tools and intelligently filters them based on your enabled plugins. With full TypeScript support, your IDE immediately knows what tools are available, what parameters they accept, and what they return.
