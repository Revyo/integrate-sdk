/**
 * Nuxt OAuth Route Adapter
 * Provides OAuth route handlers for Nuxt
 */

import { OAuthHandler, type OAuthHandlerConfig } from './base-handler.js';

// Type-only imports to avoid requiring Nuxt at build time
type H3Event = any;

/**
 * Create Nuxt OAuth route handler
 * 
 * Use this to create secure OAuth API routes in your Nuxt application
 * that handle authorization with server-side secrets.
 * 
 * @param config - OAuth handler configuration with provider credentials
 * @returns Handler function for Nuxt routes
 * 
 * @example
 * ```typescript
 * // server/api/integrate/oauth/[...all].ts
 ...
 * ```
 */
export function toNuxtHandler(config: OAuthHandlerConfig) {
