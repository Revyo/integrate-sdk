/**
 * Elysia OAuth Route Adapter
 * Provides OAuth route handlers for Elysia
 */

import { OAuthHandler, type OAuthHandlerConfig } from './base-handler.js';

// Type-only imports to avoid requiring Elysia at build time
type ElysiaContext = any;

/**
 * Create Elysia OAuth route handler
 * 
 * Use this to create secure OAuth API routes in your Elysia application
 * that handle authorization with server-side secrets.
 * 
 * @param config - OAuth handler configuration with provider credentials
 * @returns Handler function for Elysia routes
 * 
 * @example
 * ```typescript
 ...
 * ```
 */
export function toElysiaHandler(config: OAuthHandlerConfig) {
