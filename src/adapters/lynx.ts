/**
 * Lynx OAuth Route Adapter
 * Provides OAuth route handlers for Lynx cross-platform framework
 */

import { OAuthHandler, type OAuthHandlerConfig } from './base-handler.js';

/**
 * Create Lynx OAuth route handler
 * 
 * Use this to create secure OAuth API routes in your Lynx application
 * that handle authorization with server-side secrets.
 * 
 * @param config - OAuth handler configuration with provider credentials
 * @returns Handler function for Lynx routes
 * 
 * @example
 * ```typescript
 * // api/auth/[...all].ts
 ...
 * ```
 */
export function toLynxHandler(config: OAuthHandlerConfig) {
